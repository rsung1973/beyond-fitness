...
namespace WebHome.Controllers
{
    ...
    {
        public PromotionController(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        ...
        public async Task<ActionResult> CreateBonusPromotionXlsxAsync(PromotionViewModel viewModel)
        {
            ViewBag.ViewModel = viewModel;
            if(viewModel.KeyID!=null)
            {
            ...
            {
                    table.TableName = "活動方案明細表";
                ds.Tables.Add(table);

                await ds.SaveAsExcelAsync(Response, String.Format("attachment;filename={0}({1:yyyy-MM-dd HH-mm-ss}).xlsx", HttpUtility.UrlEncode("BonumPromotion"), DateTime.Now), viewModel.FileDownloadToken);
            }
