...
namespace WebHome.Controllers
{
    ...
    public class LearnerFacetController : SampleController<UserProfile>
    {
        ...
        [RoleAuthorize(new int[] { (int)Naming.RoleID.Learner, (int)Naming.RoleID.Assistant })]
        public async Task<ActionResult> LearnerIndexAsync(DateTime? lessonDate, DateTime? endQueryDate)
        {
        ...
        public async Task<ActionResult> ExchangeBonusPointAsync(int uid,int itemID,int? recipientID)
        {
            ...
            var award = new LearnerAward
            {
                ...
                ItemID = item.ItemID,
                ActorID = (await HttpContext.GetUserAsync()).UID
            };
