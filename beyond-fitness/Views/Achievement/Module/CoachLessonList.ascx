        ...
        <table id="<%= _tableId %>" class="table table-striped table-bordered table-hover display responsive no-wrap" width="100%">
            ...
            <tbody>
                <%  int PTTotalCount = 0, PTUnfinished = 0, PITotalCount = 0, PIUnfinished = 0, STTotalCount = 0, trialTotalCount = 0, selfTotalCount = 0;
                    ...
                    foreach (var c in _model.GroupBy(l => l.AttendingCoach))
                    {
                ...
                <tr>
                    ...
                    <td nowrap="noWrap" class="text-center">
                        <%  var items = c.PTLesson();
                            ...
                            if (PTCount > 0)
                            {   %>
                        <a href='javascript:showLessonList(<%= JsonConvert.SerializeObject(new { CoachID = coach.CoachID,QueryType = (int)Naming.LessonQueryType.一般課程 }) %>);'><u>(<%= PTCount %>)</u></a>
                        <%  }
                            else
                            { %>
                        --
                    ...
                    <td nowrap="noWrap" class="text-center">
                        <%  items = c.Where(l => l.TrainingBySelf == 1);
                            ...
                            if (PICount > 0)
                            {   %>
                        <a href='javascript:showLessonList(<%= JsonConvert.SerializeObject(new { CoachID = coach.CoachID,QueryType = (int)Naming.LessonQueryType.自主訓練 }) %>);'><u>(<%= PICount %>)</u></a>
                        <%  }
                            else
                            { %>
                        --
                    ...
                    <td nowrap="noWrap" class="text-center">
                        <%  items = c.Where(l => l.TrainingBySelf == 2);
                            ...
                            if (STCount > 0)
                            {   %>
                        <a href='javascript:showLessonList(<%= JsonConvert.SerializeObject(new { CoachID = coach.CoachID,QueryType = (int)Naming.LessonQueryType.在家訓練 }) %>);'><u>(<%= STCount %>)</u></a>
                        <%  }
                            else
                            { %>
                        --
                    ...
                    <td nowrap="noWrap" class="text-center">
                        <%  items = c.TrialLesson();
                            ...
                            if (trialCount > 0)
                            {   %>
                        <a href='javascript:showLessonList(<%= JsonConvert.SerializeObject(new { CoachID = coach.CoachID,QueryType = (int)Naming.LessonQueryType.體驗課程 }) %>);'><u>(<%= trialCount %>)</u></a>
                        <%  }
                            else
                            { %>
                        --
                    ...
                    <td nowrap="noWrap" class="text-center">
                        <%  items = c.Where(l => l.RegisterLesson.LessonPriceType.Status == (int)Naming.LessonPriceStatus.教練PI);

                            int selfCount = items.Count();
                            selfTotalCount += selfCount;
                            if (selfCount > 0)
                            {   %>
                        <a href='javascript:showLessonList(<%= JsonConvert.SerializeObject(new { CoachID = coach.CoachID,QueryType = (int)Naming.LessonQueryType.教練PI }) %>);'><u>(<%= selfCount %>)</u></a>
                        <%  }
                            else
                            { %>
                        --
