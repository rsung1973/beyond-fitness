...
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel">查詢體驗學員</h4>
        </div>
                    ...
                    <div class="row">
                        <section class="col col-8">
                            <label class="input">
                                ...
                                <input type="text" name="userName" maxlength="20" placeholder="請輸入體驗學員姓名" />
                            </label>
                        </section>
...
<script>
    $('#btnAttending').on('click', function (evt) {
        var $items = $('input[name="UID"]:checked');
        if ($items.length <= 0) {
            smartAlert('請勾選上課學員!!');
            return;
        }
...
<%--        $('form').prop('action', '<%= VirtualPathUtility.ToAbsolute("~/Member/ApplyGroupLessons") %>')
    ...
    $('#btnQuery').on('click', function (evt) {
        ...
        $('#userList').load('<%= Url.Action("TrialLearnerSelector","Lessons") %>', { 'userName': userName }, function () {
            hideLoading();
        });
    });
