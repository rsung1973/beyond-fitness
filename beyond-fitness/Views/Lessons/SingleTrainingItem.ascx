...
<%  if (_model.TrainingID.HasValue)
    { %>
        <tr role="row" class="">
            ...
            <td class=" expand"><span class="responsiveExpander"></span><%= _model.TrainingType.BodyParts %><%= String.IsNullOrEmpty(_model.Description) ? null : "【" + _model.Description + "】" %></td>
            <td><%= String.IsNullOrEmpty(_model.ActualTurns) ? "--" : _model.ActualTurns %>/<%= String.IsNullOrEmpty(_model.GoalTurns) ? "--" : _model.GoalTurns %></td>
            <td><%= String.IsNullOrEmpty(_model.ActualStrength) ? "--" : _model.ActualStrength %>/<%= String.IsNullOrEmpty(_model.GoalStrength) ? "--" : _model.GoalStrength %>秒</td>
            <td><%= _model.Remark %></td>
        </tr>
<%  }
    else
    { %>
        <tr class="line_btm" role="row">
            <td>
                <a href="#" data-toggle="modal" data-target="#modifyItem">
                    <i class="fa fa-pencil-square-o fa-2x text-warning btn btn-xs bg-color-orange"></i>
                </a>
                <a href="#">
                    <i class="fa fa-trash-o fa-2x btn btn-xs bg-color-redLight"></i>
