...
<script>
    $(function () {
        ...
        var PieConfig = {
            ...
            data: {
                datasets: [{
                    data: <%= JsonConvert.SerializeObject(items.Select(t=>Math.Round((_model.TrainingExecutionStage.Where(x=>x.StageID==t.StageID).Select(x=>x.TotalMinutes).FirstOrDefault() ?? 0)*100/total)).ToArray()) %>,
                    backgroundColor: [
                        'rgba(245, 166, 35, .8)',
                        'rgba(255, 78, 100, .8)',
                        'rgba(126, 211, 33, .8)',
