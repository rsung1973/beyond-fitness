...
<asp:Content ID="MainContent" ContentPlaceHolderID="MainContent" runat="server">
            ...
            <div class="card">
                <div class="body">
                    <%  
                        ...
                        {
                            Html.RenderPartial("~/Views/ContractConsole/ContractService/CourseContractExtensionView.ascx", revision);
                        }
                        else if (revision.Reason == "轉讓")
                        {
                            Html.RenderPartial("~/Views/ContractConsole/ContractService/CourseContractTransferenceView.ascx", revision);
                        }
                        else if (revision.Reason == "轉點")
                        {
                            Html.RenderPartial("~/Views/ContractConsole/ContractService/CourseContractMigrationView.ascx", revision);
                        }
                        else if (revision.Reason == "終止")
                        {
                            Html.RenderPartial("~/Views/ContractConsole/ContractService/CourseContractTerminationView.ascx", revision);
                        }
                        else if (revision.Reason == "轉換體能顧問")
                        {
                            Html.RenderPartial("~/Views/ContractConsole/ContractService/ContractConsultantReassignmentView.ascx", revision);
                        }
                        else if (revision.Reason == "其他")
                        {
                            Html.RenderPartial("~/Views/ContractConsole/ContractService/CourseContractOtherAmendmentView.ascx", revision);
                        }
                    %>
                    <div class="row clearfix">
                        <div class="col-lg-12">
                            ...
                            <%  }
                                ...
                                {   %>
                            <button type="button" class="btn btn-darkteal btn-round waves-effect float-right next" onclick="approveContractService();">確定，審核通過</button>
                            <%  }   %>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function confirmSignature() {
            showLoading();
            $.post('<%= Url.Action("ConfirmContractServiceSignature","ContractConsole",new { KeyID = _item.ContractID.EncryptKey() }) %>',
                {
        ...
        function rejectContractService() {
            showLoading();
            $.post('<%= Url.Action("DeleteCourseContract","CourseContract",new { _item.ContractID }) %>', {}, function (data) {
                hideLoading();
                if ($.isPlainObject(data)) {
